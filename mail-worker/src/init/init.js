...
const dbInit = {
    ...
    async v3_3DB(c) {
        try {
            await c.env.db.prepare(`CREATE TABLE IF NOT EXISTS merge_record (
            ...
            await c.env.db.batch([
                c.env.db.prepare(`INSERT OR IGNORE INTO perm (perm_id, name, perm_key, pid, type, sort) VALUES (39, '邮件群发', 'email:merge', 1, 2, 0.5)`),
                c.env.db.prepare(`INSERT OR IGNORE INTO role_perm (role_id, perm_id)
                    SELECT role_id, 39 FROM role WHERE role_id = 1
                    AND NOT EXISTS (SELECT 1 FROM role_perm WHERE role_id = 1 AND perm_id = 39)`)
            ]);
        } catch (e) {
